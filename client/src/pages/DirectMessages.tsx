...
import { Send, Search, MessageCircle, ArrowLeft, Check, CheckCheck, Phone, Video } from "lucide-react";
...
export default function DirectMessages() {
  ...
  // Usuario activo con quien chateamos
  ...
  const send = useMutation({
    mutationFn: () => apiRequest("POST", `/api/direct-messages/${otherId}`, { content: text }),
    onSuccess: () => {
      setText("");
  ...
  useEffect(() => {
    if (otherId) bottomRef.current?.scrollIntoView({ behavior: "auto" });
  }, [otherId, messages.length]);

  useEffect(() => {
    if (params.userId) setOtherId(params.userId);
  }, [params.userId]);
  ...
  return (
    <AppLayout title="Mensajes">
      <div className="h-[calc(100vh-4rem)] flex overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingConvs ? (
              ...
              (conversations as any[]).map((conv: any) => {
                ...
                return (
                  <button key={conv.id}
                    onClick={() => { setOtherId(other.id); navigate(`/messages/${other.id}`); }}
        ...
        <div className={`flex-1 flex flex-col min-w-0 ${!otherId ? "hidden md:flex" : "flex"}`}>
          {!otherId ? (
            ...
            <>
              ...
              <div className="border-b px-4 py-3 flex items-center gap-3 bg-background">
                <Button variant="ghost" size="icon" className="md:hidden" onClick={() => { setOtherId(""); navigate("/messages"); }}>
